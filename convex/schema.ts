...
// Simplified status options for contracts, focusing on lifecycle relevant to basic management
const contractStatusOptions = [
  "draft", // Initially created, maybe before file upload
  "pending_analysis", // File uploaded, analysis scheduled
  "active", // Analyzed, within its term
  "expired", // Past its end date
  "terminated", // Ended early
  "archived", // No longer active, kept for records
] as const; // Use 'as const' for type safety

// Simplified status for the analysis process itself
...
export default defineSchema({
  // ===== VENDORS =====
  // Stores information about the vendors you work with.
  vendors: defineTable({
    // Core Vendor Info
    name: v.string(), // Required: The vendor's name
    contactEmail: v.optional(v.string()), // Optional: Primary contact email
    contactPhone: v.optional(v.string()), // Optional: Primary contact phone
    address: v.optional(v.string()), // Optional: Vendor's address
    // Optional: Notes or general description about the vendor
    notes: v.optional(v.string()),
    // Optional: Link to vendor's website
    website: v.optional(v.string()),

    // Note: We are removing vendor status, type, category, risk, compliance etc. for simplicity.
    // Note: We are removing explicit createdBy/updatedBy user IDs for simplicity,
    //       rely on Convex's built-in _creationTime or add user IDs if auth is integrated.
  })
  // Index for quickly finding vendors by name
  .index("by_name", ["name"]),


  // ===== CONTRACTS =====
  // Stores information about each contract, linking to a vendor and the uploaded file.
  contracts: defineTable({
    // Core Contract Info
    vendorId: v.id("vendors"), // Required: Links to the 'vendors' table
    title: v.string(), // Required: A user-friendly title for the contract
    status: v.union( // Uses a simplified set of statuses
        v.literal(contractStatusOptions[0]),
        v.literal(contractStatusOptions[1]),
        v.literal(contractStatusOptions[2]),
        v.literal(contractStatusOptions[3]),
        v.literal(contractStatusOptions[4]),
        v.literal(contractStatusOptions[5]),
    ),

    // Original File Information (from upload)
    storageId: v.id("_storage"), // Required: ID for the uploaded file in Convex Storage
    fileName: v.string(), // Required: Original name of the uploaded file
    fileType: v.string(), // Required: Mime type of the uploaded file (e.g., "application/pdf")

    // Extracted Information (Populated by the analysis action)
    // All are optional because extraction might fail or data might not be present.
    extractedParties: v.optional(v.array(v.string())), // e.g., ["Company A", "Vendor B"]
    extractedStartDate: v.optional(v.string()), // Store as ISO string (YYYY-MM-DD) or similar simple format
    extractedEndDate: v.optional(v.string()),   // Store as ISO string (YYYY-MM-DD) or similar simple format
    extractedPaymentSchedule: v.optional(v.string()), // e.g., "Net 30", "Monthly on 1st"
    extractedPricing: v.optional(v.string()), // e.g., "$500/month", "Fixed fee $10,000"
    extractedScope: v.optional(v.string()), // A brief summary of the contract's scope/purpose

    // Analysis Process Tracking
    analysisStatus: v.optional(v.union( // Tracks the background analysis job
        v.literal(analysisStatusOptions[0]),
        v.literal(analysisStatusOptions[1]),
        v.literal(analysisStatusOptions[2]),
        v.literal(analysisStatusOptions[3]),
    )),
    analysisError: v.optional(v.string()), // Stores error message if analysis fails

    // Optional: Simple field for user notes about this specific contract
    notes: v.optional(v.string()),

    // Note: We are removing complex fields like contract number, type, detailed dates (effective, renewal),
    //       financials, compliance, custom fields, versioning, sharing, signatures, approvals etc.
    // Note: We are removing explicit createdBy/updatedBy user IDs for simplicity.
  })
  // Index for quickly finding contracts belonging to a specific vendor
  .index("by_vendorId", ["vendorId"])
  // Index for finding contracts by their overall status
  .index("by_status", ["status"])
  // Index for finding contracts based on the analysis job status
  .index("by_analysisStatus", ["analysisStatus"]),

  // Note: All other tables (enterprises, users, roles, permissions, templates,
  //       sessions, logs, auth details, risk, compliance, etc.) have been removed
  //       as they are not core to the requested vendor/contract management scope.
  //       Authentication/Authorization would typically be handled by integrating
  //       a service like Clerk, and user IDs could be added back to vendors/contracts
  //       if multi-user scoping is needed later.
});