...
export const getContractsSimple = query({
  ...
  handler: async (ctx, args): Promise<Doc<"contracts">[]> => {
    const result = await ctx.runQuery(api["core/contracts/contracts"].getContracts, {
      enterpriseId: args.enterpriseId,
...
export const analyzeContract = action({
  ...
  handler: async (ctx, args) => {
    try {
      ...
      const contract = await ctx.runQuery(api["core/contracts/contracts"].getContractForAnalysis, {
      ...
      await ctx.runMutation(api["core/contracts/contracts"].updateAnalysisStatus, {
        contractId: args.contractId,
        status: "processing",
      });
      ...
      await ctx.runMutation(api["core/contracts/contracts"].updateAnalysisResults, {
        contractId: args.contractId,
        analysisResult,
      });
